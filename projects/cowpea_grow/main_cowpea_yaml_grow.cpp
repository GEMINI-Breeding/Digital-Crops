...
int plant_crops(PlantArchitecture &plantarchitecture, Context &context, YAML::Node config){
    ...
    std::ifstream file("plantstring3.txt");
    if (!file.is_open()) {
    ...
    for (int i = 0; i < config["crops"].size(); i++){
        ...
        if (config["crops"][i]["crop_type"].as<std::string>() == "Cowpea"){
            ...
            plantarchitecture.buildPlantInstanceFromLibrary(plant_origin, 0);
            ...
            plantarchitecture.generatePlantFromString(plantstring, phytomer_parameters, plant_origin); 
...
int main(){
    ...
    // Plant crops
    PlantArchitecture plantarchitexture(&context);

    plant_crops(plantarchitexture, context, config);
